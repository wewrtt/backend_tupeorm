...
export class RolesService implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    ROLES_DEFAULT.forEach(async (role) => {
      const roleExisted = await this.rolesRepossitory.findOneByCondition({ where: { name: role.name } });
      if (!roleExisted) {
        ...
        role.permissions.forEach(async (permissionName) => {
          const permissionFound = await this.permissionService.findOneByConditions({
            where: {
              name: permissionName,
            },
          });
  ...
  public async findCompanyRole() {
    const blueonionHubRoles = await this.rolesRepossitory.repository.find({
      select: {
        name: true,
        id: true,
      },
      where: {
        type: RoleTypes.BlueonionHub,
      },
    });

    const blueonionHubPremiumRoles = await this.rolesRepossitory.repository.find({
      select: {
        name: true,
        id: true,
      },
      where: {
        type: RoleTypes.BlueonionPremiumHub,
      },
    });

    return {
      blueonionHubRoles: blueonionHubRoles || [],
      blueonionHubPremiumRoles: blueonionHubPremiumRoles || [],
    };
  }
